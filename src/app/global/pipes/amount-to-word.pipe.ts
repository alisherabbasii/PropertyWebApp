...
export class AmountToWordPipe implements PipeTransform {
  ...
  transform(value: any, args?: any): any {
    if (value) {
      ...
      else if (parseFloat(price) >= 0 && price.length <= 3) {
        rupees = "Price Should be in thousands PKR"
      }
      else if (parseFloat(price) > 3 && price.length <= 11) {
        for (digitIndex = price.length - 1; digitIndex >= 0; digitIndex--)
          switch (digit = price[digitIndex] - 0, nextDigit = digitIndex > 0 ? price[digitIndex - 1] - 0 : 0, price.length - digitIndex - 1) {
            ...
            case 8:
              ...
              break;
            case 9:
              words.push(handle_utlc(digit, nextDigit, "Arab"));
              break;
            case 10:
              words.push(handle_tens(digit, price[digitIndex + 1]));
              break;
            case 11:
               words.push(0 != digit ? "" + price[digitIndex + 1] + " Arab " + (0 != price[digitIndex + 1] || 0 != price[digitIndex + 2] ? " and" : " Crore") : "")
          }
        rupees = words.reverse().join("")
      } else rupees = "Price is too big. Please contact us";
      if (rupees && rupees !== "Price is too big. Please contact us" && rupees !== "Price Should be in thousands PKR")
        rupees = `${rupees} PKR`
      if (pointer != "00") {
        digitIndex = 0;
        digit = 0;
